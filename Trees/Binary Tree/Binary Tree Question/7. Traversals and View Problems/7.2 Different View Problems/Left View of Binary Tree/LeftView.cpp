...
private:
public:
    /* Method to return a list containing elements of left view of the binary tree. */
    // Time Complexity :- O(n), n is the number of nodes in a binary tree.
    // Space Complexity :- O(n), space used by queue.
    vector<int> leftView(Node *root)
    {
        /*
        Approach:- Traverse the tree using level order traversal and store the front node of each level ( from left) in our answer.
        */
        ...
        while (!q.empty())
        {
            ...
            for (int i = 1; i <= sizeOfQueue; i++)
            {
                ...
                // when i == 1, it means this frontNode is the first element/node of current level
                // Store it.
                if (i == 1)
                {
