...
public:
    ...
    vector<int> previousGreaterElement_BruteForce(vector<int> &arr, int n)
    {
    ...
    vector<int> previousGreaterElement_Optimized(vector<int> &arr, int n)
    {
        // vector to store answer.
        vector<int> ans(n,0);
        ...
        // and we know that, for the left most element the previous greater element is -1.
        st.push(-1);
...
int main(void)
{
    ...
    // Solution.
    Solution S;
    // vector<int> ans = S.previousGreaterElement_BruteForce(arr, arr.size());
    vector<int> ans = S.previousGreaterElement_Optimized(arr, arr.size());
    S.printVec(ans);
}