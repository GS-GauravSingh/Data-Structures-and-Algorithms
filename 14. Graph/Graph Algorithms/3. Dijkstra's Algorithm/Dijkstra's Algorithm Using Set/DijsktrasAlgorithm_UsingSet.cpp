...
public:
    ...
    And this is the Set Implementation of Dijkstra's Algorithm.
    ...
    */
    vector<int> dijkstra(int V, int E, int src, vector<vector<int>> &edges)
    {
        ...
        // Set will contain pairs of the type {dist, node}, where ‘dist’ indicates the currently updated value of the shortest distance from the source to the ‘node’.
        // The node with minimum distance will present at the top of the set, because in set minimum element is present at the top and rest of the element is stored in ascending order.
        set<pair<int, int>> s; // set store unique values and minimum value is present at the top and rest of the element is stored in ascending order.
        ...
        // Push the 'src' node into the set with the distance 0.
        ...
        while (!s.empty())
        {
            // Take out the top element from the set.
            pair<int, int> topElement = *(s.begin()); // s.begin will return the iterator that point to the first element of the set, so we need to dereference this iterator to get the values.

            // Pop the top element of the set.
            s.erase(topElement);
            ...
            for (auto pair : adjList[node])
            {
                ...
                if ((dist + edgeWeight) < distance[neighbor])
                {
                    if (distance[neighbor] != INT_MAX)
                    {
                        ...
                        /* If distance of neighbor is "INT_MAX", it means that the neighbor has not been visited before and it is the first time that someone has reached to this neighbor. This is because the initial value of all distances in the distance array is set to INT_MAX, which represents an unreachable node.

                        Question: But what if the distance of neigbor in not "INT_MAX" ?
                        Answer: If the distance of the neighbor is not INT_MAX, it means someone has already reached that node with the distance of "distance[neighbor]".

                        So I'am sure that there would be a pair of {distance[neighbor], neighbor} in the set, because someone reached this neighbor at a distance of "distance[neighbor]".

                        And we are reaching this neighbor again with the shorter distance and pushing this pair into the set so if this pair goes to the set. There is no point keeping that previous pair in the set.

                        So, Using set we can erase already existing path.
                        */
