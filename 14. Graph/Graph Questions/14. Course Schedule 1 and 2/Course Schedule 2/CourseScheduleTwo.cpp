...
public:
    ...
    */
    ...
    If the graph is a DAG then return one of the possible topological ordering, Graph is not cyclic, it means it is possible to finish all the tasks. So return the order in which we perform all the tasks.

    Otherwise if the graph is not a DAG then return an empty vector, it means it is not possible to finish all the tasks.
    */
    ...
    vector<int> findOrder(int n, int m, vector<vector<int>> prerequisites)
    {
        ...
        if (topoSort.size() == n)
        {
            ...
            // So return the order in which we perform all the tasks.
            return topoSort;
        }

        // Else, return ans empty vector, it means graph is cyclic and it is not possible to finish all the tasks.
        return {};
    }
};
...
int main(void)
{
    ...
    // Solution.
    Solution S;
    vector<int> order = S.findOrder(N, P, prerequisites);
    for (auto i : order)
    {
        cout << i << " ";
    }
