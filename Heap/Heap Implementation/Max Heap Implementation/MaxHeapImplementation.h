...
public:
    ...
    index:-          0  1  2  3  4  5
    array/vector = {-1,60,50,40,30,20}; // blocking the 0th index.

    For node present at 1 index i.e., 60.
    node = 60, (i = 1).

    Left child of 60 = (2 * i)th index,
    => 2 * 1 = 2, at index 2, in vector, 50 is present and the left child of 60 is 50.

    Right child of 60 = (2 * i + 1)th index,
    => 2 * 1 + 1 = 3, at index 3, in vector, 40 is present and the right child of 60 is 40.

    Parent Node of 60 = (i/2)th index,
    => 1/2 = 0.5, when index is less than 0, then it means you are finding the parent of root node and root node has no parent.
    => 0.5 is not a valid index, so there is no parent of 60.
    ...
    => In vector, value present at index 4 to 5 are 40 and 50, so 40 and 50 are leaf nodes and you can verify it through the above tree preview.
    ...
    Suppose, We have to insert, let's say 55 in our MAX Heap. Follow the below steps to insert,
    ...
    ----------------- DRY RUN ------------------------
    Step 1: insert that value at the end of our vectori.e., push_back(value).


    after push_back(55) operation in vector,
    index:           0  1  2  3  4  5   6
    ...
    Now, to place 55 to its correct position, compare the inserted value with its parent node. In case of Max Heap value of parent Node >= value of its child nodes and In case of Min Heap value of parent Node <= value of its child nodes.
    ...
    Parent node of 55 is present at (i/2)th index
    Index of 55 is 6, so i = 6.
    ==> i/2 = 6/2 = 3, Parent node of 55 is present at 3rd index and 40 is present at index 3.
    ==> Parent of 55 is 40.

    Now, This is a Inplementation of Max Heap and In case of Max Heap value of parent Node >= value of its child nodes.

    Comparing Inserted Value (55) with its parent 40.
    ==> Parent Node is 40 and Child Node is 55.
    ==> Child node is > Parent Node, it means 55 is not at it correct position, so, swap it with its parent node.
    ...
    */
    ...
    /* Method to Push and Element into the Max Heap. */
    ...
    Let's start deletion, as we know that in Heap we always perform deletion from root node. So, lets delete root node i.e., 60.
    ...
    Now, in our vector we clearly see that, the last nodes is 40 and the root node is 60.
    ...
    index:           0  1  2  3  4  5  6
    array/vector = {-1,40,50,55,30,20, 60};
    visualize tree:-
                                            40
                                          /    \
                                        50     55
    ...
    For placing Root node to its correct position, we have to compare root node with its child nodes and In case of Max Heap value of parent Node >= value of its child nodes.

    Root Node = 40, (i = 1).
    Left Child of 40 is at (2*i)th index i.e., 2*1 = 2, left child of 40 is present at index 2 and at index 2, 50 is present so the left child of 40 is 50.

    Right Child of 40 is at (2*i+1)th index i.e., 2*1+1 = 3, right child of 40 is present at index 3 and at index 3, 55 is present so the right child of 40 is 55.
    ...
    */
    ...
    /* Method to Pop Element from Max Heap. We always perform Deletion from Root Node. */
