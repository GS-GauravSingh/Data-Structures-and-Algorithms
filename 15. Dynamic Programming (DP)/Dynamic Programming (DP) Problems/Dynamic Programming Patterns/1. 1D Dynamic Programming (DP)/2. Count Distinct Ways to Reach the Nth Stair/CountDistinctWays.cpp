...
private:
    ...
    int solveSpaceOptimized(int n)
    {
        ...
        for (int i = 2; i <= n; i++)
        {
            nThStair = (zeroThStair + oneThStair) % mod;
...
public:
    /* Question: An integer value 'n' is given, where 'n' represents the number of stairs. Initially you are at the '0th' stair and you need to reach the 'Nth' stair. From each stair you can either move/climb one step or two step. Return the number of distinct ways to reach the nth stair. Return answer in modulo 1e9+7. */
...
int main(void)
{
    // Our task is to find the number of distinct ways to reach the nth stair.
    int n = 3;

    // Solution.
    Solution S;
    cout << S.countDistinctWays(n);
